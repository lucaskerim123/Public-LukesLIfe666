...
export default async function TrackerPage() {
  ...
  const { data: sessions } = await supabase.from('drug_tracker_sessions').select('*').order('date_start', { ascending: false })
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          {isAdmin && <Link href="/tracker/new" className="border border-amber-900/60 text-amber-800 hover:bg-amber-950/30 px-4 py-2 text-[11px] font-mono tracking-widest uppercase transition-colors">+ New Session</Link>}
        </div>
              ...
              <div className="border border-zinc-800 hover:border-zinc-700 bg-zinc-950 px-4 py-4 flex items-center justify-between transition-colors">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <span className={`text-[10px] font-mono px-2 py-0.5 tracking-widest uppercase ${!s.date_end ? 'text-amber-700 bg-amber-950/30 border border-amber-900/30' : 'text-zinc-500 bg-zinc-800 border border-zinc-700'}`}>{!s.date_end ? `ONGOING – DAY ${daysUp(s.date_start)}` : `DAY ${daysUp(s.date_start, s.date_end)}`}</span>
                  </div>
